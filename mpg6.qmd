...
This can be rewritten using the Bayes factor BF10:
...
Modern genomics routinely tests **MANY** hypotheses:

-   20,000+ genes in differential expression: [Storey et
    al](https://www.pnas.org/doi/10.1073/pnas.1530509100)\
-   Millions of variants in GWAS: [Stephens et
    al](https://academic.oup.com/biostatistics/article-abstract/18/2/275/2557030?redirectedFrom=fulltext)\
-   Compounded potential interactions across multiple phenotypes: [Urbut
    al](https://www.nature.com/articles/s41588-018-0268-8)

**The consequence**: Many "significant" findings are actually false
positives: but which ones?
...
true effects are rare. How do you know $pi0$?vThe Bayesian solutions ...
...
$$f_1(z) = \int \mathcal{N}(z|\mu, se^2) \mathcal{N}(\mu|0, \sigma^2) d\mu = \mathcal{N}(0, se^2 + \sigma^2)$$
 This is a normal distribution with increased *true effect* variance for tests where $H_1$ is true.

:::notes
This is the marginal likelihood after integrating over possible true effect sizes, where:
$\sigma^2$ represents the variance of true effects
Larger $\sigma^2$ means we expect larger true effects
The marginal likelihood will be more spread out (heavier tails)
Therefore, large observed effects are more likely under this distribution
:::

## Prior, Likelihood, Post: Drawing Interlude

(Board)


:::notes
$p(\theta) = \sum_k \pi_k N(\mu_0^k, \sigma_0^k)$
$p(z|\theta) = N(\theta, se^2)$
$p(\theta|z) = \sum_k \pi_k^* N(\mu_1^k, \sigma_1^k)$
$\pi_k^* \propto \pi_k \times p(z|\mu_0^k, \sigma_0^k + se^2)$
:::
...
3.  **Correlation structure**: Account for sharing of noise (mashr)
...
1. Effects are shrunk adaptively based on both their size and the overall distribution of effects
2. Small observed effects are shrunk more heavily toward zero because they're more likely to be noise
3. Large observed effects undergo less shrinkage because they're more likely to be real
4. The amount of shrinkage is learned from the data itself through: Estimation of π₁ (proportion of non-null effects)
Estimation of σ (scale of true effects)
5. The local false discovery rate (LFDR) determines shrinkage amount for each effect
:::
## Empirical Bayes Approach
...
- $\sigma^2_1$ and $\sigma^2_2$: trait variances\
- $\rho$ is the correlation coefficient\
- $\rho\sigma_1\sigma_2$ represents the covariance.

## More math

The multivariate normal density is then:

$$p(\mathbf{x}|\mathbf{U}\_k) = \frac{1}{2\pi|\mathbf{U}_k|^{1/2}}
...
1.  When ($\rho$ = 0):
    ...
    -   approaches univariate shrinkage but ..
2.  When ($\rho$ $\neq$ 0):
    -   Effects are correlated, 'spread' of cloud \propto $\rho$
    -   Slope determined by relative *variance*
...
-   **Only considering only information in one subgroup ignores the
    *hints* we get from abound**
...
:::notes
even under an 'incoorrect prior', you can't help but come to the truth with adeuqate data
:::

## Simple Example:

Why TB was a geneticist: Imagine tracking minor allele frequency:
...
::: notes
sum of a and b is like precision, higher concentration at mean mean is
a/a+b
:::
...
## Understanding the q Update: At the Individual Level

**Individual 1**: A C A

$\log(P(Data|Pop1)) = \log(f_1^A) + \log(f_1^C) + \log(f_1^A)$

$\log(P(Data|Pop2)) = \log(f_2^A) + \log(f_2^C) + \log(f_2^A)$

Total for Pop1 = $L_{11} + L_{12} + L_{13}$\
Total for Pop1 = $L_{21} + L_{22} + L_{23}$

::: notes
Here, we're updating the ancestry proportions (`q`) for Individual
    1.  For each marker, we calculate the likelihood of the observed allele under each population's allele frequency.
...
to get the updated parameters for the
Dirichlet distribution. The result is a new estimate of Individual 1's ancestry proportions, reflecting both the prior and the observed data.
This process is repeated for each individual in the dataset
:::

## Step 2: Dirichlet Update

$$q_1 \sim \text{Dirichlet}(\alpha1 + \sum{L1}, \alpha2 +\sum{L2})$$ where:

**-** $\sum{L1} = L_{11} + L_{21} + L_{31}$ (added to $\alpha_1$)\
**-** $\sum{L2} = L_{12} + L_{22} + L_{32}$ (added to $\alpha_2$)


::: callout-note
The Dirichlet update incorporates log likelihood contributions from both
populations to estimate how much of Individual 1's ancestry comes from
each population.
:::

:::notes
The key difference between this and just adding counts is
Simple case: We KNOW which population (just one!) each allele belongs to
STRUCTURE case: We DON'T know which population each allele belongs to, so we:Calculate how likely the sequence is under each population
Use these likelihoods to update our beliefs about population membership
Then use these membership probabilities (q) to update allele frequencies (f). Conjugate Dirichlet-Multinomial Special Case:
Prior: Dirichlet(α₁, α₂, α₃)
Likelihood: Multinomial(n₁, n₂, n₃)
Posterior: Dirichlet(α₁ + n₁, α₂ + n₂, α₃ + n₃)
The beauty of conjugate priors is that this complex calculation: adding counts in the conjugate case IS doing likelihood × prior, it's just a mathematically convenient way to get there!
:::
...
**Problem**: The meta analysis example, combining evidence across
...
These come from true effects $\theta_i$ that vary around the overall
effect $\mu$ with variance $\tau^2$ We put priors on $\mu$ (using
$\mu_0, \sigma_0^2$) and $\tau^2$ (using $a, b$) The key is that
$\tau^2$ represents how much the true effects vary between studies,
while $\sigma_i^2$ represents the uncertainty within each study. $\mu_0$
and $\sigma_0^2$ are just prior parameters for our belief about the
overall effect.
:::
...
::: notes
Start with informative priors when possible **- Use previous studies\
- Incorporate functional annotations\
- Consider evolutionary constraints Report posterior probabilities, not
just p-values** - $P(\text{association} | \text{data})$ is more
interpretable than $P(\text{data} | \text{no association})$\
- Provides direct probability statements about hypotheses Use conjugate
models for computational efficiency **- Beta-Binomial for allele
frequencies\
- Dirichlet-Multinomial for haplotype frequencies\
- Normal-Normal for quantitative traits Consider mixture models for
complex data** - Population structure\
- Heterogeneous effect sizes\
- Multiple causal variants Apply decision theory for optimal
designs\*\* - Balance false positives and false negatives\
