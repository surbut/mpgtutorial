...
set.seed(123)
n_tests <- 10000
pi1 <- 0.05  # 5% true effects
sigma <- 2    # Effect size parameter

# Generate true and null effects separately
...
calc_post_prob <- function(z, pi1=0.05, sigma=2) {
  f0 <- dnorm(z, 0, 1)
...
plot_data <- data.frame(
  ...
  post_prob = c(
    calc_post_prob(z_null),
    calc_post_prob(z_true)
  )
)

# Plot
ggplot(plot_data, aes(x = z_score, y = post_prob, color = is_true)) +
