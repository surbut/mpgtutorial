...
**Key insight**: With a realistic prior of 1/1000, a "significant" p-value of 0.001 only gives ~50% posterior probability of true association! (app interlude)
...
- Prior probability of true association: ~1/1000 per variant  
...
Bayes' Theorem for GWAS:

P(True|Z) = P(Z|True)×P(True) / [P(Z|True)×P(True) + P(Z|Null)×(1-P(True))]

Where:

- P(Z|Null) ~ N(0,1) 
- P(Z|True) ~ N(0,1+σ²) 
- P(True) = prior (e.g., 1/1000) 
...
# Likelihoods
null_like = dnorm(z, 0, 1)         # = 0.0001334
alt_like = dnorm(z, 0, sqrt(1+4))  # = 0.0037

# Bayes Factor
BF = alt_like/null_like            # ≈ 27.7

# Posterior
post_odds = BF * prior/(1-prior)   # ≈ 0.0277
post_prob = post_odds/(1+post_odds) # ≈ 0.027 (2.7%)
```

```{r}
# Function to calculate posterior for different priors
calc_posterior <- function(z, prior, sigma2) {
  null_like = dnorm(z, 0, 1)
  alt_like = dnorm(z, 0, sqrt(1+sigma2))
  BF = alt_like/null_like
  post_odds = BF * prior/(1-prior)
  post_odds/(1+post_odds)
}

# Compare different priors
priors = c(1/10000, 1/1000, 1/100)
z = 4
results = sapply(priors, function(p) calc_posterior(z, p, 4))

# Plot results
ggplot() +
  geom_col(aes(x=factor(priors), y=results)) +
  labs(x="Prior", y="Posterior Probability",
       title="Impact of Prior Choice on Posterior Probability",
       subtitle="For Z-score = 4") +
  theme_minimal()
```
...
ggplot(plot_data, aes(x = Group, y = Estimate, color = Method, group = Method)) +
  ...
  scale_color_manual(values = c("True" = "black", "No Pooling" = "red", "Complete Pooling" = "blue", "Hierarchical" = "green")) +
...
# BMA prediction
pred_bma <- pred1 * post_probs[1] + pred2 * post_probs[2] + pred3 * post_probs[3] + pred4 * post_probs[4]
...
ggplot() +
  ...
  scale_color_manual(values = c("Linear" = "blue", "Quadratic" = "green", "Cubic" = "purple", "Trigonometric" = "orange",
                               "BMA" = "red", "True" = "black")) +
  scale_size_manual(values = c("Linear" = 0.7, "Quadratic" = 0.7, "Cubic" = 0.7, "Trigonometric" = 0.7,
                              "BMA" = 1.2, "True" = 1.2)) +
  scale_linetype_manual(values = c("Linear" = "solid", "Quadratic" = "solid", "Cubic" = "solid", "Trigonometric" = "solid",
...
plot_df$Study <- factor(plot_df$Study, 
                      levels = c(paste0("Study ", 1:n_studies), "Fixed Effects", "Random Effects", "Bayesian Robust"))
