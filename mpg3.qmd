...
## Mixture Models: Intuition and Building Blocks
...
## Mixture Models in Genomics: Key Applications

::::: columns
::: {.column width="50%"}
**1. Population Structure**
- Components = ancestral populations
- Weights = admixture proportions
- Example: STRUCTURE software

**2. Gene Expression Clustering**
- Components = cell types/states
- Weights = proportion of cells in each state
- Example: Single-cell RNA-seq analysis

**3. Effect Size Distribution**
- Components = null/causal variants
- Weights = proportion of causal variants
- Example: Bayesian fine-mapping
:::
...
## EM Algorithm: How Mixture Models Are Fitted
...
**M-step**: Update parameters using weighted data
...
## Population Structure as a Mixture Model
...
## Common Conjugate Priors in Genomics

| Data Model | Conjugate Prior | Application |
|------------|-----------------|-------------|
| Binomial | Beta | Allele frequencies |
| Multinomial | Dirichlet | Haplotype frequencies |
| Normal (known variance) | Normal | Effect sizes |
| Normal (unknown variance) | Inverse-Gamma | Variance components |
| Poisson | Gamma | Mutation rates |
...
## Bayesian Decision Theory: Beyond p < 0.05

::::: columns
::: {.column width="50%"}
**Moving from significance to utility**:

1. **Define utilities/costs**:
   - Benefit of true discovery
   - Cost of false discovery
   - Cost of missed discovery
   - Cost of additional samples

2. **Calculate expected utility**:
   - For each possible decision
   - Weighted by posterior probabilities

3. **Choose action that maximizes utility**:
   - May differ by context
   - Explicitly incorporates costs/benefits
:::

::: {.column width="50%"}
```{r}
#| fig-height: 6
# Create a visualization of decision theory
set.seed(234)

# Define parameters
effect_sizes <- seq(-0.5, 0.5, length.out = 100)
prior_mean <- 0
prior_sd <- 0.2

# Calculate prior density
prior_density <- dnorm(effect_sizes, prior_mean, prior_sd)

# Define utility functions
# 1. Symmetric loss (declare significant if |effect| > 0.2)
symmetric_utility <- ifelse(abs(effect_sizes) > 0.2, 1, 0)

# 2. Asymmetric loss (higher utility for positive effects)
asymmetric_utility <- ifelse(effect_sizes > 0.2, 2,
                           ifelse(effect_sizes < -0.2, -1, 0))

# 3. Continuous utility (proportional to effect size)
continuous_utility <- effect_sizes^2 * sign(effect_sizes)

# Create data frame for plotting
utility_df <- data.frame(
  EffectSize = rep(effect_sizes, 3),
  Utility = c(symmetric_utility, asymmetric_utility, continuous_utility),
  UtilityType = rep(c("Symmetric", "Asymmetric", "Continuous"), each = length(effect_sizes))
)

# Calculate expected utilities
expected_symmetric <- sum(symmetric_utility * prior_density) / sum(prior_density)
expected_asymmetric <- sum(asymmetric_utility * prior_density) / sum(prior_density)
expected_continuous <- sum(continuous_utility * prior_density) / sum(prior_density)

# Create plot
ggplot(utility_df, aes(x = EffectSize, y = Utility, color = UtilityType)) +
  geom_line(size = 1.2) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
  scale_color_brewer(palette = "Set1") +
  labs(title = "Utility Functions for Different Decision Rules",
       subtitle = paste0("Expected utilities: Symmetric = ", round(expected_symmetric, 2),
                       ", Asymmetric = ", round(expected_asymmetric, 2),
                       ", Continuous = ", round(expected_continuous, 2)),
       x = "Effect Size", 
       y = "Utility",
       color = "Utility Type") +
  theme(legend.position = "bottom")
...
- Handle missing data
- Quantify uncertainty in network structure
...
edges <- data.frame(
  from = c("SNP1", "SNP2", "Gene1", "Gene1", "Gene2", "Gene3", "Gene4"),
  to = c("Gene1", "Gene2", "Gene3", "Gene4", "Gene3", "Trait", "Trait"),
  prob = c(0.85, 0.92, 0.78, 0.65, 0.71, 0.88, 0.76)
)

# Add the final edge
edges <- rbind(edges, data.frame(from = "Trait", to = "Outcome", prob = 0.94))

# Create a function to draw arrows
draw_arrow <- function(from_x, from_y, to_x, to_y, prob, color = "black") {
  # Calculate arrow coordinates
  arrows(from_x, from_y, to_x, to_y, length = 0.1, angle = 30, 
         lwd = 1 + 2 * prob, col = color)
  
  # Add probability label
  text_x <- (from_x + to_x) / 2
  text_y <- (from_y + to_y) / 2
  text(text_x, text_y, labels = prob, cex = 0.8, col = "darkblue")
}
...
plot_network <- function() {
  ...
  for (i in 1:nrow(edges)) {
    ...
    draw_arrow(nodes$x[from_idx], nodes$y[from_idx],
...
## Case Study: Bayesian Fine-Mapping
...
**Bayesian approach**:
- Calculate posterior probability of causality
- Report credible sets (e.g., 95% probability)
- Incorporate functional priors
:::
